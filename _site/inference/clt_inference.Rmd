...
We can use the `visualize` function from the `infer` package to plot the CLT - 
based null null distribution. We indicate the test statistic with a red vertical 
line and shade the region corresponding to the $p$-value in red.
...
Instead of following the steps above every time we want to do a hypothesis test, 
we can use the `t_test` function from the `infer` package to conduct a $t$-test.
To do this we need to specify the following:
...
2. Calculate the standard error and store the result as `se`.
...
3. Use the summary statistics and standard error to calculate the test 
statistic.
...
6. Use the code chunk below to replicate the analysis above using the
...
7. In additional to hours worked per week, the GSS asked if the respondent 
...
```{r quiz2, echo=FALSE}
quiz(
  question("Report the p-value from Exercise #7 and provide an interpretation 
