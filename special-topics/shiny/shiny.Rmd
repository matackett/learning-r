...
from R. ^[Shiny. (2020). Retrieved from https://shiny.rstudio.com/.] You can host standalone apps on a webpage or embed them in R Markdown 
...
<figcaption>Figure 1: Shiny app on laptop / desktop<br/>
*Source*: https://shiny.rstudio.com/tutorial/</figcaption>
</center>
</figure>
...
<figcaption>Figure 2: Shiny app on remote server<br/>
*Source*: https://shiny.rstudio.com/tutorial/</figcaption>
</center>
</figure>
...
```{r ui_shiny_framework, eval=FALSE}
# Define UI for application that draws a histogram
ui <- fluidPage(
...
`renderPlot({})` of your server function and replace it with the code below.
Don't forget to load `tidyverse` at the beginning of your script file.
...
Run your app. It should now look similar to the app below.
...
You'll notice that changing the fill color in the select box from Red to
Blue does not change the color of the rectangles in the histogram, yet. We'll
enable this functionality in the next section.
...
```{r echo=FALSE}
decorate("flair_ex") %>% 
  flair("bins = input$bins")
```

```{r flair_ex, eval=FALSE}
output$distPlot <- renderPlot({
  faithful %>% 
    ggplot(aes(x = waiting)) +
    geom_histogram(bins = input$bins, fill = "red", color = "black", alpha = .3) +
    labs(x = "Waiting time", y = "Frequency", title = "Old Faithful Geyser") +
    theme_minimal(base_size = 16)
})
```
...
Now it's time to build on what you learned in the last sections. Add some more
reactivity to your app with a check box that previews the data used to
create the histogram. Follow the steps below.
...
Checking "Preview data" has no affect on the app. 
In the next step, you'll link this newly created widget to R code in the server function to enable reactivity.
...
If you would like to publish your app for others to see,
[shinyapps.io](https://www.shinyapps.io/) provides an easy, secure, and
scalable platform to deploy your shiny apps on the web. Their free account
option will allow you to host five applications for a total of 25 active user
hours per month.
...
2. Click the publish button in RStudio and follow the directions provided by
   shinyapps.io to link your account.
   
<figure>
<center>
<img src="images/publish_app.png" height="150" width="300"/>
</center>
</figure>

3. Share the URL to your app with others!
